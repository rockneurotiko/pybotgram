...
def run(msg, matches):
	...
	if not fromCur in currencies:
		return "ERROR: currency \"" + fromCur + "\" is not recognized"
	
	if not toCur in currencies:
		return "ERROR: currency \"" + toCur + "\" is not recognized"
	
	returnText = "Oops, an error occurred."

	url = "https://www.google.com/finance/converter?a=" + amount + "&from=" + fromCur + "&to=" + toCur 	
	html = urllib2.urlopen(url).read()
	moneyMatches = re.search("<span class=bld>([\d.]+)", html)
	if moneyMatches:
		returnText = "{} {} is {} {}".format(amount, fromCur, moneyMatches.group(1), toCur)
	
	return returnText

__info__ = {
    "description": "Currency converter",
    "usage": ["!m[oney] (from currency) (amount) (to currency)"],
